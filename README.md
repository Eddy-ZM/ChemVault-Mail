...
桌面端安装后会在安装目录保留 `ChemVault Mail.exe`，出现在开始菜单，并在安装时让用户选择是否创建桌面快捷方式。应用启动后会检查 GitHub Releases 更新源；发现新版本时提示下载，下载完成后由用户选择何时重启并更新。

下载入口：`/download/windows`

当前安装包命名规则：`ChemVault-Mail-Setup-0.1.5.exe`
