...
const dbInit = {
	...
	async v3_9DB(c) {
		try {
			...
			console.warn(`跳过 Mail Client 表：${e.message}`);
		}
		...
		for (const sql of emailColumns) {
			try {
				...
				console.warn(`跳过 Mail Client 字段：${e.message}`);
			}
		}
	},
	...
	async v3_5DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN cloudflare_access_external_perms TEXT NOT NULL DEFAULT 'all-email:query';`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
	...
	async v3_4DB(c) {
		try {
			await c.env.db.prepare(`ALTER TABLE setting ADD COLUMN apple_api_base_url TEXT NOT NULL DEFAULT '';`).run();
		} catch (e) {
			console.warn(`跳过字段：${e.message}`);
		}
	},
